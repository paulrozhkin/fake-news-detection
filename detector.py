...
def detect():
    real_data = pd.read_csv('data_small/True.csv')
    fake_data = pd.read_csv('data_small/Fake.csv')
    ...
    torch.save(model.state_dict(), "smol_after_train.pt")
    ...
    #test_news(fake1, device, model, tokenizer)
    #test_news(true1, device, model, tokenizer)
